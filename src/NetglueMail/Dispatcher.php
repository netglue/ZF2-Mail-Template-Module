...
class Dispatcher implements EventsCapableInterface, EventManagerAwareInterface
{
    ...
    public function createMessage($messageName, array $options = [], $viewModel = null)
    {
        ...
        if (!is_null($viewModel) && !$viewModel instanceof ModelInterface) {
            throw new Exception\InvalidArgumentException(sprintf(
                'Expected a view model or an array received %s',
                gettype($viewModel)
            ));
        }

        $html = $this->templateService->renderTemplate($messageName, $viewModel);
        $text = $this->templateService->renderTextTemplate($messageName, $viewModel);
     ...
     * @return void
     */
    protected function addAttachments(Mail\Message $message, array $options)
    {
